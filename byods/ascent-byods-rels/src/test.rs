...
use ascent::{ascent, ascent_run, Lattice};
...
impl PartialOrd for PalindromeR {
   fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> { Some(self.len().cmp(&other.len())) }
}

impl Ord for PalindromeR {
   fn cmp(&self, other: &Self) -> std::cmp::Ordering { self.len().cmp(&other.len()) }
}
...
#[test]
fn test_linear_palindrome() {
   let s = "abacdfgdcaba";
   let res = ascent_run! {
       #[ds(crate::linear::linear)]
   ...
   let longest_p_str = s[l_s..l_e].to_string();
   println!("longest_p_str: {:?}", longest_p_str);
}
